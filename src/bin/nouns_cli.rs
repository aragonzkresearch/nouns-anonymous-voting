...
use ethers::types::{Address, NameOrAddress, StorageProof, TransactionRequest};
use futures::TryStreamExt;

use nouns_anonymous_voting::voter::Voter;
use nouns_anonymous_voting::{wrap, wrap_into, BBJJ_Ec, BBJJ_Fr, BN254_Fr, VoteChoice, Wrapper};
...
async fn main() {
    // parse the CLI input
    let (global_param, cli_command) = cli().unwrap_or_else(|err| {
        eprintln!("Error: {}", err);
        std::process::exit(1);
    });
    ...
    let client = SignerMiddleware::new(eth_connection.clone(), wallet);
    ...
    match cli_command {
        ...
        CliCommand::Vote(
            ...
            let chain_id = eth_connection
                .get_chainid()
                .await
                .map_err(|e| "Could not get chainid".to_string())
                .unwrap();
...
async fn reg_key(
    ...
    let contract = ZKRegistry::new(contract_address, client);
    ...
    let x_tx = x_register_request
        .send()
        .await
        .map_err(|e| format!("Error sending X coordinate registration tx"))?;
    println!(
    ...
    let y_tx = y_register_request
        .send()
        .await
        .map_err(|e| format!("Error sending Y coordinate registration tx"))?;
...
async fn create_process(
    ...
    let tx = create_process_request
        .send()
        .await
        .map_err(|e| format!("Error sending createProcess tx"))?;
...
async fn vote(
    ...
    let tx = submit_vote_request
        .send()
        .await
        .map_err(|e| format!("Error sending vote tx"))?;
...
fn cli() -> Result<(GlobalCliParams, CliCommand), String> {
    ...
    if let Some(matches) = matches.subcommand_matches("reg-key") {
        ...
        let key_to_reg =
            parse_bbjj_prk(key_to_reg).map_err(|e| "Invalid key to register in ZKRegistry")?;
    ...
    if let Some(matches) = matches.subcommand_matches("vote") {
        ...
        let nft_owner_prk =
            parse_bbjj_prk(nft_owner_prk).map_err(|e| "Invalid nft owner private key")?;
    ...
    if let Some(matches) = matches.subcommand_matches("tally") {
        ...
        let tcls_prk = parse_bbjj_prk(tcls_prk).map_err(|e| "Invalid tcls private key")?;
...
fn parse_bbjj_prk(key_to_reg: &String) -> Result<PrivateKey, String> {
    PrivateKey::import(
...
/// Example: `1h` (1 hour)
/// Example: `1m` (1 minute)
fn parse_duration<T: Into<String>>(s: T) -> Duration {
    let s = s.into();
...
fn parse_tlcs_pbk<T: Into<String>>(s: T) -> Result<BBJJ_Ec, String> {
    let s = s.into();
    let mut chars = s.chars();
...
fn parse_u256<T: Into<String>>(s: T) -> Result<U256, String> {
    let s = s.into();
