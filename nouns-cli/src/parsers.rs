...
pub(crate) fn parse_ipfs_hash<T: Into<String>>(s: T) -> Result<H256, String> {
    ...
    if s_cid.codec() != 0x55 {
        return Err("IPFS CID must be raw binary".to_string());
    }

    let s_multihash = s_cid.hash();

    if s_multihash.code() != 0x12 {
        return Err("IPFS CID's underlying must be a sha2-256 hash".to_string());
    }

    let s_bytes = s_multihash.digest();

    if s_bytes.len() != 32 {
        return Err("IPFS CID digest does not have the required length (32 bytes)".to_string());
    }

    let s_h256 = <[u8; 32]>::try_from(s_bytes.deref())
        .map_err(|e| format!("Could not convert vector to byte array: {}", e))?
        .into();
